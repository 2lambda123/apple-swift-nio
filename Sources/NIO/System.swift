...
// Declare aliases to share more code and not need to repeat #if #else blocks
    ...
    func sysWritev_wrapper(fd: CInt, iov: UnsafePointer<iovec>?, iovcnt: CInt) -> CLong {
        CLong(writev(fd, iov, iovcnt)) // cast 'Int32' to 'CLong'
    }
...
internal enum Posix {
        ...
        static let UIO_MAXIOV = Int(Glibc.UIO_MAXIOV)
        static let SHUT_RD = CInt(Glibc.SHUT_RD)
        static let SHUT_WR = CInt(Glibc.SHUT_WR)
        static let SHUT_RDWR = CInt(Glibc.SHUT_RDWR)
        #if os(Android)
            static let IPTOS_ECN_NOTECT = CInt(CNIOLinux.IPTOS_ECN_NOTECT)
        #else
            static let IPTOS_ECN_NOTECT = CInt(CNIOLinux.IPTOS_ECN_NOT_ECT)
        #endif
        static let IPTOS_ECN_MASK = CInt(CNIOLinux.IPTOS_ECN_MASK)
        static let IPTOS_ECN_ECT0 = CInt(CNIOLinux.IPTOS_ECN_ECT0)
        static let IPTOS_ECN_ECT1 = CInt(CNIOLinux.IPTOS_ECN_ECT1)
        static let IPTOS_ECN_CE = CInt(CNIOLinux.IPTOS_ECN_CE)
    #else
        ...
        internal static func socket(domain: NIOBSDSocket.ProtocolFamily, type: NIOBSDSocket.SocketType, protocol: CInt) throws -> CInt {
            try syscall(blocking: false) {
                sysSocket(domain.rawValue, type.rawValue, `protocol`)
            }.result
        }
                                    ...
                                    len: UnsafeMutablePointer<socklen_t>?) throws -> CInt?
        {
            ...
            if case let .processed(fd) = result {
                return fd
            } else {
                return nil
            }
