...
struct DatagramVectorReadManager {
                               ...
                               reportExplicitCongestionNotifications: Bool) -> ReadResult
    {
        ...
        for i in 0 ..< messageCount {
            ...
            let readBytes = Int(messageVector[i].msg_len)
            ...
            if reportExplicitCongestionNotifications {
                let controlMessagesReceived =
                    UnsafeControlMessageCollection(messageHeader: messageVector[i].msg_hdr)
...
private extension UnsafeMutableBufferPointer {
    ...
    func deinitializeAndDeallocate() {
        ...
        let rawPointer = basePointer.deinitialize(count: count)
